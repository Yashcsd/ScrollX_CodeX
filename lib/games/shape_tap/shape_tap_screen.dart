...
class _Shape {
  ...
  Color color;
  bool tapped;
  _Shape({required this.x, required this.y, required this.type, required this.color, this.tapped = false});
}
...
class _ShapeTapScreenState extends State<ShapeTapScreen> {
  ...
  @override
  void initState() { super.initState(); _target = _types[_rng.nextInt(_types.length)]; _start(); }
  @override
  ...
  void _tap(int idx) {
    ...
    if (s.tapped) return;
    if (s.type == _target) {
      setState(() { s.tapped = true; _score += 20; });
      Future.delayed(const Duration(milliseconds: 300), () {
        if (mounted) setState(() => _shapes.remove(s));
      });
    } else {
      setState(() { _lives--; if (_lives <= 0) _endGame(); });
    }
  ...
  void _endGame() {
    ...
    context.read<UserProvider>().recordGameResult(
      gameId: 'shape_tap', gameName: 'Shape Tap',
      score: _score, timeTakenSeconds: 45, won: _score >= 120,
    );
  }
  ...
  Widget _drawShape(_Shape s, double w, double h) {
    ...
    switch (s.type) {
      case 'circle':
        shape = Container(width: size, height: size,
          decoration: BoxDecoration(shape: BoxShape.circle, color: s.color.withOpacity(0.85)));
      case 'square':
        shape = Container(width: size, height: size,
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(10), color: s.color.withOpacity(0.85)));
      case 'triangle':
        shape = CustomPaint(size: const Size(size, size),
          painter: _TrianglePainter(s.color.withOpacity(0.85)));
      default: // star
        shape = Text('⭐', style: TextStyle(fontSize: size * 0.8, color: s.color));
    }
    return Positioned(
      left: s.x * w - size/2,
      top:  s.y * h - size/2,
      child: GestureDetector(
        ...
        child: AnimatedOpacity(opacity: s.tapped ? 0 : 1,
          duration: const Duration(milliseconds: 200), child: shape)));
  }
      ...
      body: Container(
        ...
        child: SafeArea(child: Stack(children: [
          ...
          if (_showResult) GameResultOverlay(
            ...
            onRetry: () => setState(() {
              _shapes=[]; _score=0; _lives=3; _timeLeft=45; _showResult=false;
              _target=_types[_rng.nextInt(_types.length)]; _start();
            }),
          ),
        ])),
      ),
